...
const NavButtons = () => {
    ...
    const renderMenuItems = () => {
        ...
        return Array.from({ length: numberOfButtons }, (_, i) => {
            ...
            const buttonStyle = [
                ...
                styles.menuItem,
                style,
                i === 0 ? { backgroundColor: '#2CEAA3' } : { backgroundColor: '#2CEAA3' },
                {
                    shadowColor: '#000',
                    shadowOffset: {
                        width: 0,
    ...
    return (
        <View style={styles.container}>
            <View onClick={() => menuOpen ? toggleMenu() : null} style={{ backgroundColor: menuOpen ? 'rgba(0, 0, 0, .5)' : 'rgba(0, 0, 0, 0)', display: menuOpen ? 'block' : 'none', ...styles.blackScreen }} ></View>
            <TouchableOpacity
                style={[styles.button, menuOpen ? styles.menuButtonOpen : styles.menuButton]}
                onPress={toggleMenu}>
                <Animated.View style={menuButtonRotation}>
                    {menuOpen ? <AntDesign name="close" size={24} color="white" /> : <Feather name="trending-up" size={24} color="white" />}
                </Animated.View>
            </TouchableOpacity>
            {menuOpen && (
                <TouchableOpacity
